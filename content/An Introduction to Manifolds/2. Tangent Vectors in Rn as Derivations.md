---
date: 2024-07-23
title: 2. Tangent Vectors in $\R^n$ as Derivations
---
...
**Proof:** Notice that by $\R$-linearity, $D(c)=cD(1)$ for all $r\in\R$. So it suffices to show that $D(1)=0$. By making use of Leibniz's rule,
$$
	D(1) = D(1)1 + 1D(1) = 2D(1)
$$
...
The vector $X_p\in T_p(\R^n)$ can be expressed as a linear combiation
$$
...
$$
Notice that if $X$ is a $C^\infty$ vector field, and $f$ is a $C^\infty$ function on $U$, then $fX$ is a $C^\infty$ vector field on $U$. Hence, the set of all $C^\infty$ vector fields on $U$, denoted $\mathfrak{X}(U)$, a module over the ring $C^\infty(U$).
