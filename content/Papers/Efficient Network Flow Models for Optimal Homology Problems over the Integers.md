...
$$
	(\delta_d\boldsymbol{\varphi})(\mathbf{c}) = \boldsymbol{\varphi}(\partial_d \mathbf{c})~\text{ for }\mathbf{c}\in\cal{C}_d(\cal{K}).
$$
...
- A **flow** is another name for a $d$-cycle
	- **Flow conservation** condition: $\partial_d\mathbf{z}=0$ which is equivalent to $\mathbf{z}(\delta_d\sigma)=0$ for all $\sigma\in\cal{K}_{d-1}$
- A **coflow** is another name for a $d$-cocycle
	- **Circulation conservation** condition: $\delta_{d+1}\mathbf{f}=0$, i.e., $\mathbf{f}(\partial_{d+1}\tau)=0$ for all $\tau\in\cal{K}_{d+1}$
...
- **Homology Signature:**
	...
	- Let $\mathring{\mathbf{w}}_0=\partial_{d+1}\cal{K}+\sum_{l=1}^{\beta_d}\mathring{\mathbf{w}}_l$
...
$$
	|\cal{c}(\mathbf{x})|=\sum_{e\in\cal{K}_d}\cal{c}(e_i)\cdot|\mathbf{x}(e_i)|\ge0~\text{ and }~|\alpha{c}(\boldsymbol{\pi})|=\sum_{\tau_j\in\cal{K}_{d+1}}\alpha(\tau_j)\cdot|\boldsymbol{\pi}(\tau_j)|\ge0
$$
